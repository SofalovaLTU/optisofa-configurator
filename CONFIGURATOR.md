...
- **2026-07-06 (9)** — **PABAIGA: Mood tiesios sofos + 15 „-open" (išskleistų lovų) variantų.** Mood sofa (`SET5.obj` = tikras Mood 3v sofa, ne scena; TAG išmestas). Visi sofa-lova/šezlongų „-open" (miego būsenos) variantai perkonvertuoti iš „unfolded/_Open" šaltinių. `reconv.py` papildytas: papildomi assimp flag'ai (SortByPType/FindDegenerates) + **trimesh fallback** sugadintiems OBJ (bonnie-chaise-open). `drive.py` resilientiškas (try/except per job). **VISA ne-modulinė kolekcija dabar perkonvertuota iš Optisofa šaltinio.** GLB_V→e.
- **2026-07-06 (8)** — **PERKONVERTAVIMAS: sofos-lovos + kampinės + šezlongai (26 modeliai).** Atidžiai parinkti FOLDED (sofos būsenos) šaltiniai (ne „unfolded"/„rozłożony"), o šezlongams — PILNAS sofa+šezlongas („Sofa with chaise longue"), NE vien šezlongo modulis (senoji Mood klaida). Perkonvertuota: sofos-lovos (Blues, Bonnie, Esme, Harris, Jane, Joleen, Juliett, Nora, Salma, Slay + chaise variantai), kampinės (Esme, Jane, Karin, Mood), šezlongai (Harris, Mood, Nora, Salma, Slay, Uma). Matmenų check — visi OK (šezlongų gylis ~1.6m patvirtina pilną L formą). Verifikuota: Mood su šezlongu = pilnas sofa+šezlongas (ne vien šezlongas); Harris sofa-lova = suskleista + ąžuolo kojos; Karin kampinė = suskleista + juodos metalinės kojos. LIKO (nebūtina): -open (išskleistos lovos) variantai + Mood tiesios sofos (SET5).
- **2026-07-06 (7)** — **Masinis PERKONVERTAVIMAS: foteliai + pufai + sofos (46 modeliai) iš Optisofa šaltinio.** Seni modeliai buvo sulieti į vieną „Body" (mano senas konverteris) — prasta kokybė. Sukurtas `map_sources.py` (fuzzy mapper: config kodas → šaltinio failas OPTISOFA ASORTIMENTAS aplanke, pagal kolekciją+tipą; rel-path matching, „N vietų"→sėdynės) ir `drive.py` matmenų sanity-check (perspėja jei mastelis/orientacija ne pagal JSON W/D/H). Perkonvertuota IŠSAUGANT atskirus mesh'us (pagalvės, kojos, piping): visi **foteliai** (Blues, Bonnie, Emilly, Esme, Harris, Ines, Jane, Joleen, Juliett, Mia, Nora, Salma, Uma), **pufai** (Aya, Cake, Cup, Juliett, Slay, Toby, Vibe), **sofos** (Blues, Emilly, Esme, Harris, Ines, Jane, Joleen, Juliett, Juliett2, Karin, Salma, Slay, Uma + 2v). Verifikuota naršyklėje (Salma/Uma sofos, Esme fotelis, Cake pufas) — Optisofa kokybė: apvalios plombuotos pagalvės, teisingos kojos (metalas/ąžuolas), audinio spalva=swatch, minkšti šešėliai. GLB_V→c (cache-bust). LIKO perkonvertuoti: sofos-lovos, kampinės, šezlongai (folded-state, atsargiau) + Mood (SET5 šaltinis tikrintinas).
- **2026-07-06 (6)** — **Audinio spalvos tikslumas + matinis apmušalas.** Naudotojas: pasirinkus audinį (pvz. Bison velvet) sofa atrodė blizgi/pilka, ne swatch spalvos. Priežastis: aukštas velvet **sheen (0.55, baltas)** + sustiprintas apšvietimas → baltos „karštos" atošvaitos užgoždavo bazinę spalvą. **Pataisa:** sheen sumažintas (velvet 0.55→0.16, kiti ~0.03), `specularIntensity 1.0→0.28`, `envMapIntensity 0.55→0.35`, `clearcoat 0` — matinis audinys, spalva = swatch. Apšvietimas sušvelnintas (key 2.15→1.7, rim 1.25→0.75). Verifikuota: Bison → matinis rudas (kaip swatch), Linen → kreminis su matoma faktūra; abu 100% atitinka audinio nuotrauką.
- **2026-07-06 (5)** — **x100 atvaizdavimo kokybė (apšvietimas + šešėliai) + Emilly kojos.** Naudotojas: baldai atrodo kampuoti, netikroviški šešėliai. **Studijinis 3-taškų apšvietimas:** key (2.15, šiltas, castShadow) + fill (0.55, vėsus) + rim/back (1.25, atskiria nuo fono) + soft hemi (0.50); ekspozicija 0.82→0.92; aštresnis PMREM IBL (0.04→0.02). **Šešėliai:** `updateGroundShadow()` (kviečiama `frameAll`) fituoja key šešėlio kamerą prie modelio ribų (aštrūs HD šešėliai) + `radius 8/blurSamples 24` minkšti kraštai + `normalBias`. **Minkštas kontaktinis šešėlis** (radial gradient „grounding blob" po modeliu, keičia mastelį pagal footprint) — produkto-foto pojūtis. Tinklelis 0.16→0.05 (vos matomas). Rezultatas verifikuotas: Emilly/Karin/Molly — minkšti tikroviški šešėliai, apvalios formos. Taikoma VISIEMS modeliams vienu kartu. **Emilly sofa-lova perkonvertuota** (Sofabed Emily.obj: `leg-wood`+`feet`) → ąžuolo kojos; regex papildytas `feet`.
